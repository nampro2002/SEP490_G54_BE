...
public interface MonthlyQuestionRepository extends JpaRepository<MonthlyRecord, Integer> {
    ...
    @Query("SELECT w.monthNumber FROM MonthlyRecord w WHERE  w.appUserId.id = ?1 AND w.monthNumber =?2 AND  w.monthlyRecordType = 'NEW_MONTH_MARK' order by  w.monthNumber desc limit 12")
    List<Integer> find12ByAppUserAndMonthNumber(Integer userId, Integer monthNumber);
