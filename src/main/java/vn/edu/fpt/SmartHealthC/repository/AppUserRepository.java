...
public interface AppUserRepository extends JpaRepository<AppUser, Integer> {
    //name contain search
    @Query("SELECT u FROM AppUser u WHERE u.webUser.id = ?1 AND LOWER(u.name) LIKE %?2% and u.accountId.isActive=true and u.accountId.isDeleted = false")
...
//    @Query(value = "SELECT a.id, a.dob, a.gender, a.height, a.medical_specialist_note, a.name, a.phone_number, a.weight, a.account_id, a.cic, a.web_user_id FROM account ac JOIN app_user a ON ac.id = a.account_id LEFT JOIN web_user w ON a.web_user_id = w.id LEFT JOIN account acc ON w.account_id = acc.id WHERE ac.is_active = 1 AND (a.web_user_id IS NULL OR acc.is_deleted = true) ORDER BY ac.id ASC", nativeQuery = true)
    @Query(value = "SELECT a.id, a.dob, a.gender, a.height, a.medical_specialist_note, a.name, a.phone_number, a.weight, a.account_id, a.cic, a.web_user_id FROM account ac JOIN app_user a ON ac.id = a.account_id LEFT JOIN web_user w ON a.web_user_id = w.id LEFT JOIN account acc ON w.account_id = acc.id WHERE ac.is_active = 1 AND ac.is_deleted = 0 AND (a.web_user_id IS NULL OR acc.is_deleted = true) ORDER BY ac.id ASC", nativeQuery = true)
